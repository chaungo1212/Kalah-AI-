...
public class AIGame extends JFrame {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AIGame frame = new AIGame("6", "4");//set this default so that while doing it is easier to run
	...
	public AIGame(String houses_per, String seeds_per) {
		finished = false;
		...
		button_reset.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				timer.stop();
				dispose();
				Game frame = new Game(houses_per, seeds_per);
				frame.setVisible(true);
		...
		while(finished == false){
			...
			timer.start();
				...
				if(judge.win_store1 == false && judge.win_store2 == false){
					...
					System.out.print("AI 2's move indices(including all free turn):");
